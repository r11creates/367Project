...
//  ss is strong scaling, sse is strong scaling efficiency, ws is weak scaling, wse is weak scaling efficiency  
int main( int argc, char **argv )
{
    int n[MAX_ENTRIES],i,count=0,num,p[MAX_ENTRIES];
    double t[MAX_ENTRIES],slope[MAX_ENTRIES-1],ss[MAX_ENTRIES],sse[MAX_ENTRIES],ws[MAX_ENTRIES],grade,sse_avg,ws_avg;
    ...
    if( find_option( argc, argv, "-h" ) >= 0 )
    {
        ...
        printf( "-v to specify what to autograde (serial,openmp,mpi) \n" );
        return 0;
    }
    ...
    if (strcmp(autoname,"openmp")==0 || strcmp(autoname,"mpi")==0){
      if(fread){
